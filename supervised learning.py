...
class Preprocess(): #this class is for preprocessing and labeling the documents
    ...
    def preprocess(self, raw_text):
        ...
        tokenized_word_s= []
        for cd in range(len(tokenized_word)):
...
class Transform(): #this class is for transorming the documents to bow , tfidf and also applying the n_grams
    def __init__(self, pp_corpus, pp_df_corpus, n_gram = 2 ):
...
class Visualization():
    ...
    def word_freq(self):
        ...
        freqdist.plot(100)
    def word_cloud_author(self): #errors for an specific author for instance 'a'
        random_corpus = []
        for i in range(50):
            random.seed(1)
            random_number = np.random.randint(0, 200)
            random_corpus.append(pp_corpus[random_number])
        random_corpus1=[]
        random_corpus1.append(' '.join(str(''.join(str(x) for x in v)) for v in random_corpus))
        random_corpus2 = " ".join(str(x) for x in random_corpus1)
        random_corpus_word = random_corpus2.split(' ')
        count=0
        list_count =[]
        author_repeat=[]
        for error in error_words:
            for rand_word in random_corpus_word:
                if error == rand_word :
                    count+=1
                    author_repeat.append(rand_word)
            list_count.append(count)
            count=0
        random.shuffle(author_repeat)    
        author_repeat_corpus = " ".join(str(x) for x in author_repeat)
        author_cloud = WordCloud(max_font_size=60).generate(author_repeat_corpus)
        plt.figure(figsize=(16,12))
        plt.imshow(author_cloud, interpolation="bilinear")
        plt.axis("off")
        plt.show()
        
        freqdist = nltk.FreqDist(author_repeat)
        plt.figure(figsize=(20,5))
        freqdist.plot(100)
...
eris = Erorr_Analysis()
mislocs , misclassification = eris.misclassified(eris.y_test_tfidf, y_pred_dt_tfidf, eris.cm_dt_tfidf)
corpus_repeat, repeating_words , feature = eris.get_features(x_test_tfidf, mislocs)
...
viz.ten_fold_v(train.ten_fold(train.clf_KNN_tfidf, x_train_tfidf, y_train_tfidf))
